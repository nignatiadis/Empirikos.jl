...
@testset "Folded Distribution" begin
    ...
    @testset "PDF" begin
        ...
        @test Empirikos.pdf(fmx, 0.0) ≈ 2 * pdf(mix, 0.0) atol = 1.0e-12
        @test all(
            y -> isapprox(
                Empirikos.pdf(fmx, y), 0.3 * (pdf(comp[1], y) + pdf(comp[1], -y)) + 0.7 * (pdf(comp[2], y) + pdf(comp[2], -y));
    ...
    @testset "CDF and logcdf" begin
        ...
        @test isapprox(Empirikos.logcdf(folded_asy, 4.0), log(cdf(asy, 4.0) - cdf(asy, -4.0)); atol = 1.0e-12, rtol = 1.0e-10)
        ...
        @test all(
            y -> isapprox(
                Empirikos.cdf(fmx, y), 0.3 * (cdf(comp[1], y) - cdf(comp[1], -y)) + 0.7 * (cdf(comp[2], y) - cdf(comp[2], -y));
                rtol = 1.0e-10, atol = 1.0e-12
            ), ys_pos
        )
        @test all(
            y -> isapprox(
                Empirikos.logcdf(fmx, y), log(0.3 * (cdf(comp[1], y) - cdf(comp[1], -y)) + 0.7 * (cdf(comp[2], y) - cdf(comp[2], -y)));
                rtol = 1.0e-10, atol = 1.0e-12
            ), ys_pos
        )
    end
    ...
    @testset "Folded Normal quantile" begin
        ...
        @test isapprox(x0, sqrt(Empirikos.quantile(Chisq(1), 0.5)); atol = 1.0e-8)
        ...
        for (μ, σ, q) in (
            ...
            @test x ≥ 0
            @test isapprox(Empirikos.cdf(d, x), q; atol = 1.0e-8, rtol = 1.0e-8)
        end
    ...
    @testset "Folded LocationScale TDist quantile" begin
        folded = Empirikos.Folded( 2.0 * TDist(5.0))

        # Test quantile/cdf roundtrip
        for q in [0.1, 0.5, 0.9]
            @test cdf(folded, quantile(folded, q)) ≈ q atol = 1.0e-10
        end

        # Test μ ≠ 0 throws
        @test_throws ArgumentError quantile(Empirikos.Folded(LocationScale(1.0, 1.0, TDist(5.0))), 0.5)
    end
    ...
    @testset "LogPDF" begin
        ...
        @test isapprox(Empirikos.logpdf(folded_asy, 3.0), (log(Empirikos.pdf(asy, 3.0) + Empirikos.pdf(asy, -3.0))); atol = 1.0e-8, rtol = 1.0e-8)
    end
    ...
    @testset "Folded Normal Computations" begin
        @testset "likelihood_distribution for FoldedNormalSample" begin
            ...
            @test isapprox(mean(Empirikos.unfold(dbn)), μ; atol = 1.0e-10, rtol = 1.0e-10)
            @test isapprox(std(Empirikos.unfold(dbn)), σ; atol = 1.0e-10, rtol = 1.0e-10)
        end
        @testset "marginalize(FoldedNormalSample, Normal)" begin
            ...
            @test isapprox(mean(Empirikos.unfold(got)), mean(pred); atol = 1.0e-10, rtol = 1.0e-10)
            @test isapprox(std(Empirikos.unfold(got)), std(pred); atol = 1.0e-10, rtol = 1.0e-10)
        end
        ...
        @testset "marginalize(FoldedNormalSample, Mixture{Normal})" begin
            ...
            for i in 1:3
                @test isapprox(mean(Empirikos.unfold(got.components[i])), mean(pred.components[i]); atol = 1.0e-11, rtol = 1.0e-11)
                @test isapprox(std(Empirikos.unfold(got.components[i])), std(pred.components[i]); atol = 1.0e-11, rtol = 1.0e-11)
            end
            ...
            @test all(
                y -> isapprox(
                    Empirikos.pdf(got, y), pdf(pred, y) + pdf(pred, -y);
                    rtol = 1.0e-11, atol = 1.0e-12
                ), ys
            )
            @test all(
                y -> isapprox(
                    Empirikos.cdf(got, y), cdf(pred, y) - cdf(pred, -y);
                    rtol = 1.0e-11, atol = 1.0e-12
                ), ys
            )
        ...
        @testset "posterior(FoldedNormalSample, Normal)" begin
            ...
            @test isapprox(sum(post.prior.p), 1.0; atol = 1.0e-12)
            ...
            @test isapprox(mean(post.components[1]), mpos; atol = 1.0e-10, rtol = 1.0e-10)
            @test isapprox(std(post.components[1]), s; atol = 1.0e-10, rtol = 1.0e-10)
            @test isapprox(mean(post.components[2]), mneg; atol = 1.0e-10, rtol = 1.0e-10)
            @test isapprox(std(post.components[2]), s; atol = 1.0e-10, rtol = 1.0e-10)
            sp = sqrt(σ^2 + τ0^2)
            wpos = pdf(Normal(μ0, sp), +z)
            wneg = pdf(Normal(μ0, sp), -z)
            wsum = wpos + wneg
            @test isapprox(post.prior.p[1], wpos / wsum; atol = 1.0e-12, rtol = 1.0e-12)
            @test isapprox(post.prior.p[2], wneg / wsum; atol = 1.0e-12, rtol = 1.0e-12)
        end
        ...
        @testset "posterior(FoldedNormalSample, Mixture{Normal})" begin
            ...
            in_w_exp = [
                let wpos = pdf(Normal(μ[k], spred[k]), +z),
                        wneg = pdf(Normal(μ[k], spred[k]), -z)
                        [wpos / (wpos + wneg), wneg / (wpos + wneg)]
                end for k in 1:K
            ]

            @test isapprox(probs(post), out_w_exp; rtol = 1.0e-12, atol = 1.0e-12)
            ...
            for k in 1:K
                ...
                for j in 1:2
                    ...
                    @test isapprox(μhat, μexp; rtol = 1.0e-11, atol = 1.0e-11)
                    @test isapprox(σhat, σexp; rtol = 1.0e-11, atol = 1.0e-11)
                    @test isapprox(ŵ, wexp; rtol = 1.0e-12, atol = 1.0e-12)
                end
            end
        end
        @testset "marginalize(FoldedNormalSample, Uniform)" begin
            ...
            @test all(y -> isapprox(pdf(got, y), pdf_expected(y); rtol = 1.0e-8, atol = 1.0e-10), ys)
            @test all(y -> isapprox(cdf(got, y), cdf_expected(y); rtol = 1.0e-8, atol = 1.0e-10), ys)
            ...
            @test all(y -> isapprox(pdf(got_asym, y), pdf_expected(y); rtol = 1.0e-8, atol = 1.0e-10), ys2)
            @test all(y -> isapprox(cdf(got_asym, y), cdf_expected(y); rtol = 1.0e-8, atol = 1.0e-10), ys2)
        end
    end
end
