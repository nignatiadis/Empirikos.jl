...
infty_floc = Empirikos.InfinityNormDensityBand(;a_min=-3.0,a_max=3.0,α=0.05)
...
prostate_kde_plot = begin
	...
	plot!(prostate_marginal_plot, [-3.0;3.0], seriestype=:vline,
...
amari_kde_locmix = Empirikos.AMARI(
...
amari_kde_scalemix = Empirikos.AMARI(
...
postmean_ci_amari_locmix = confint.(amari_kde_locmix, postmean_targets, Zs)

# ╔═╡ a262fe94-4bf7-11eb-1153-6f76128cbcd0
postmean_locmix_plot = begin
	postmean_locmix_plot = plot(ts, postmean_ci_kde_locmix, label="Gauss-F-Loc",
	...
	plot!(postmean_locmix_plot, ts, postmean_ci_amari_locmix, label="AMARI",
...
postmean_ci_amari_scalemix = confint.(amari_kde_scalemix, postmean_targets, Zs)
...
postmean_scalemix_plot = begin
	...
	plot!(postmean_scalemix_plot, ts, postmean_ci_amari_scalemix,
...
lfsr_ci_amari_locmix = confint.(amari_kde_locmix, lfsrs, Zs)
...
lfsr_locmix_plot = begin
	lfsr_locmix_plot = plot([-3;3], [0.5; 0.5], seriestype=:line,
		linestyle=:dot, label=nothing, color=:lightgrey,
        xguide = L"z",
		yguide=L"\PP{\mu \geq 0 \mid Z=z}", size=(380,280))
	plot!(lfsr_locmix_plot, ts, lfsr_ci_kde_locmix,
		label="Gauss-F-Loc", fillcolor=:darkorange, fillalpha=0.5, ylim=(0,1))
	plot!(lfsr_locmix_plot, ts, lfsr_ci_dkw_locmix,
		label="DKW-F-Loc",show_ribbon=false, alpha=0.9, color=:black)
	plot!(lfsr_locmix_plot, ts, lfsr_ci_amari_locmix,
		label="AMARI",show_ribbon=true, fillcolor=:blue, fillalpha=0.4)
end
...
lfsr_ci_amari_scalemix = confint.(amari_kde_scalemix, lfsrs, Zs)
...
lfsr_scalemix_plot = begin
	...
	plot!(lfsr_scalemix_plot, ts, lfsr_ci_dkw_scalemix,
		label="DKW-F-Loc",show_ribbon=false, alpha=0.9, color=:black)
	plot!(lfsr_scalemix_plot, ts, lfsr_ci_amari_scalemix,
		label="AMARI",show_ribbon=true, fillcolor=:blue, fillalpha=0.4)
end
...
# ╔═╡ 3e986c36-79a1-4ffd-9d2b-4bc96f2a5fa8
md"## Sensitivity Analysis and goodness-of-fit"

# ╔═╡ c6758a61-c32f-4ec7-8336-085f6dadb96d
Random.seed!(1)

# ╔═╡ 3950ca34-f8eb-4450-985f-fdb47a1b6f8b
n_half = ceil(Int64, nobs(Zs)/2)

# ╔═╡ e61af6d2-e4df-4c9a-bb2e-40e8e706f36d
train_idx = sample(1:nobs(Zs), n_half, replace=false);

# ╔═╡ 8a936d80-c61f-4d22-a1f1-c891d0063ee2
test_idx = setdiff(1:nobs(Zs), train_idx);

# ╔═╡ d0c37eca-4c5e-42ba-970a-4d1fe27d33ea
Zs_train = Zs[train_idx]

# ╔═╡ 5ef18ef6-0d4a-4846-a1ec-8dfcc4168588
Zs_test = Zs[test_idx]

# ╔═╡ 506eb209-b761-48a2-8a31-0fe8c98a9802
τs = [0.02; 0.1; 0.25; 0.5; 0.55]

# ╔═╡ 80f5d5de-0d75-4a6e-8143-8e3f3b48af43
gcal_np = DiscretePriorClass(-4:0.005:4)

# ╔═╡ fb6464e6-3a4d-4609-9b96-37edc2a6021f
npmle_fit = fit(NPMLE(convexclass=gcal_np, solver=Mosek.Optimizer), Zs_train)

# ╔═╡ c09e057e-24b2-4486-a497-e4908dcf2881
res_list  = Vector{Any}(undef, 5);

# ╔═╡ c5ae00f4-c078-4ece-9ebf-90ea8e5b724a
postmean_target = Empirikos.PosteriorMean(StandardNormalSample(2.0))

# ╔═╡ 6501119c-338c-4931-a09e-38b5b3f8a81d
lfsr_target = Empirikos.PosteriorProbability(StandardNormalSample(2.0),
                          Interval(0,nothing))

# ╔═╡ a1b52f7a-4a53-485f-b246-fe008322c6a4
for (i, τ) in enumerate(τs)

    gcal_loc = MixturePriorClass(Normal.(-3:0.002:3, τ))

    npmle_loc_fit = fit(NPMLE(convexclass=gcal_loc, solver=Mosek.Optimizer), 
						Zs_test)
	
    e_value = exp(loglikelihood(Zs_test, npmle_fit.prior) -
		          loglikelihood(Zs_test, npmle_loc_fit.prior))

    floc_tmp = FLocalizationInterval(flocalization = fitted_infty_floc,
                convexclass = gcal_loc;
                solver = quiet_mosek)

    local floc_fit_lfsr
    local ci_lfsr
    local floc_fit_postmean
    local ci_postmean
    try
        floc_fit_lfsr = fit(floc_tmp, lfsr_target, Zs)
        ci_lfsr = confint(floc_fit_lfsr)

        floc_fit_postmean = fit(floc_tmp, postmean_target, Zs)
        ci_postmean = confint(floc_fit_postmean)
    catch
        floc_fit_lfsr = "error"
        ci_lfsr = "error"
        floc_fit_postmean = "error"
        ci_postmean = "error"
    end
    res_list[i] = (floc_fit_lfsr = floc_fit_lfsr, ci_lfsr = ci_lfsr,
                   floc_fit_postmean = floc_fit_postmean, ci_postmean = ci_postmean,
                   e_value=e_value)
end

# ╔═╡ 24b4a3d6-2e7d-4f84-bf84-6fe97b24e93a
DataFrame(τ = τs, 
	      SLR = getproperty.(res_list, :e_value),
          Lfsr_CI = getproperty.(res_list, :ci_lfsr),
		  Postmean_CI = getproperty.(res_list, :ci_postmean))

# ╔═╡ 4c812df1-fc00-4f42-a50c-4d08ca5b576b
us = -3:0.01:3

# ╔═╡ 8925c810-b292-4dc6-ba4d-3e040d8b8689
_plots = [plot(us,
   [pdf.(res_list[j].floc_fit_lfsr.g1, us) pdf.(res_list[j].floc_fit_lfsr.g2, us)],
   title=L"\tau = %$(τs[j])", color = [:darkblue :purple],
   linealpha = [0.6 0.8], xguide=L"\mu", yguide=L"g(\mu)")
   for j in 1:4];

# ╔═╡ fdb0fc1a-2b8b-4d6c-b848-47d145308875
worst_case_plots = plot(_plots..., layout=(1,4),size=(900,200), legend=false)

# ╔═╡ 40d68d1a-f93f-45e6-8f0e-3c25af40c411
# savefig(worst_case_plots, "worst_case_priors.tikz")

# ╔═╡ 7fe7a76b-fe38-4019-b068-d471123d1c9a
postmean_ci_kde_locmix[findfirst(ts.==2.0)] #sanity check 1

# ╔═╡ 81fd9a91-35dc-48dc-bb79-f01ea110d952
lfsr_ci_kde_locmix[findfirst(ts.==2.0)] #sanity check 2
...
[[Empirikos]]
deps = ["CSV", "DataStructures", "Distributions", "Intervals", "JuMP", "KernelDensity", "LinearAlgebra", "LinearFractional", "MathOptInterface", "Optim", "ParameterJuMP", "QuadGK", "Random", "RecipesBase", "Reexport", "Setfield", "Statistics", "StatsBase", "UnPack"]
git-tree-sha1 = "6f38a107e365b57d2a8f7561dd6b8aca46a5f421"
...
git-tree-sha1 = "d6f7fc5da94fbc9b68a999eb4da32ef832e1d766"
...
git-tree-sha1 = "883be630ab4a25991037c2a2079d7170a5a8908e"
uuid = "6405355b-0ac2-5fba-af84-adbd65488c0e"
version = "1.1.3"

[[MosekTools]]
deps = ["MathOptInterface", "Mosek", "Printf"]
git-tree-sha1 = "56f7c254a9d3769bda929e05f1db2d19d8aa3375"
...
git-tree-sha1 = "a73cb3215747dc6d42a71efd760933c9209c3d2b"
